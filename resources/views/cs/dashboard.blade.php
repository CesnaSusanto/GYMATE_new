...
<body class="bg-gray-100 font-sans">
    <div class="flex h-screen">
        ...
        <div class="w-64 bg-gray-800 text-white flex flex-col justify-between">
            <div>
                ...
                <div class="p-4">
                    <a href="#" id="showMembers" class="sidebar-item block py-3 px-4 rounded hover:bg-gray-700 bg-gray-100 text-gray-800 font-semibold mb-2" data-target="memberList">
                        List Member
                    </a>
                    <a href="#" id="showTrainers" class="sidebar-item block py-3 px-4 rounded hover:bg-gray-700" data-target="trainerList">
                        List Trainer
                    </a>
                </div>
            </div>
        ...
        <div class="flex-1 p-8 overflow-y-auto">
            <div class="bg-white p-8 rounded-lg shadow-md">
                ...
                {{-- SECTION LIST MEMBER --}}
                {{-- Hapus 'active' default di sini, biar JS yang tentukan --}}
                <div id="memberList" class="content-section"> 
                    ...
                    <form action="{{ route('cs.dashboard') }}" method="GET" class="mb-6 flex gap-4">
                        {{-- PERBAIKI NAMA INPUT: dari "search" menjadi "member_search" --}}
                        <input type="text" name="member_search" placeholder="CARI MEMBER BERDASARKAN NAMA / USERNAME" 
                               ...
                               value="{{ $memberSearch }}"> {{-- Menggunakan $memberSearch --}}
                        ...
                        @forelse($members as $member)
                            <div class="flex items-center justify-between bg-gray-50 border border-gray-200 p-4 rounded-md shadow-sm">
                                ...
                                <div class="flex space-x-2">
                                    ...
                                    {{-- Tombol Delete (yang sudah ada atau placeholder) --}}
                ...
                <div id="trainerList" class="content-section">
                    ...
                    <form action="{{ route('cs.dashboard') }}" method="GET" class="mb-6 flex gap-4">
                        <input type="hidden" name="active_tab" value="trainerList">
                        ...
                        @forelse($trainers as $trainer)
                            <div class="flex items-center justify-between bg-gray-50 border border-gray-200 p-4 rounded-md shadow-sm">
                                <div>
                                    ...
                                    {{-- Hapus tampilan Spesialisasi dan Jadwal --}}
                                    {{-- <p class="text-sm text-gray-600">Spesialisasi: {{ $trainer->spesialisasi }}</p> --}}
                                    {{-- <p class="text-sm text-gray-600">Jadwal: {{ $trainer->jadwal_pelatihan }}</p> --}}
                                </div>
                                <div class="flex space-x-2">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Dapatkan kedua parameter pencarian dari URL
            ...
            function showSection(targetId) {
                ...
                sidebarItems.forEach(item => {
                    item.classList.remove('bg-gray-100', 'text-gray-800', 'font-semibold');
                    item.classList.add('text-white');
                });
                const activeItem = document.querySelector(`.sidebar-item[data-target="${targetId}"]`);
                if (activeItem) {
                    activeItem.classList.add('bg-gray-100', 'text-gray-800', 'font-semibold');
            ...
            sidebarItems.forEach(item => {
                item.addEventListener('click', function(e) {
                    ...
                    // Bersihkan kedua parameter pencarian dari URL saat berpindah tab
                    const currentUrl = new URL(window.location.href);
                    currentUrl.searchParams.delete('member_search');
                    currentUrl.searchParams.delete('trainer_search');
            ...
            // Prioritaskan tab trainer jika ada parameter trainer_search
            if (trainerSearchParam) {
                showSection('trainerList');
            } else if (memberSearchParam) {
                // Jika tidak ada trainer_search tapi ada member_search
                showSection('memberList');
            } else {
                // Tampilkan "List Member" secara default jika tidak ada pencarian
                showSection('memberList');
            }
        });
    </script>
